...
impl Platform {
    pub fn monitor_from_cursor(app: &tauri::AppHandle<Wry>) -> TextSnapResult<Option<Monitor>> {
    ...
    pub fn xcap_monitor_from_cursor(
        app: &tauri::AppHandle<Wry>,
    ) -> TextSnapResult<Option<XCapMonitor>> {
        let cursor_pos: tauri::PhysicalPosition<f64> = app.cursor_position()?;
        let monitors = XCapMonitor::all()?;

        let monitor = monitors.into_iter().find_map(|m| {
